...
public:
    ...
    const int thread_num = std::min(fpga_num, 4);     
    ...
    std::vector<std::vector<int>> sorted_fpga_ids = std::vector<std::vector<int>>(thread_num, std::vector<int>());    
...
private:
    ...
    void resort_hypernodes(const std::vector<std::pair<int, int>>& connection_counts) {
    ...
    void sort_hypernodes() {
        ...
        resort_hypernodes(connection_counts);
    }

    void resort_fpgas(const std::vector<std::pair<int, int>>& connection_counts, int thread_id) {
        std::vector<bool> fpga_used(fpga_num, false);
        for(int i = 0; i < fpga_num; ++i) {
            if(i == 0){
                sorted_fpga_ids[thread_id].push_back(connection_counts[thread_id].first);
                fpga_used[connection_counts[thread_id].first] = true;
            }
            else{
                ...
                for(int j = 0; j < fpga_num; ++j) {
                    ...
                    if(comm_cost[sorted_fpga_ids[thread_id].back()][connection_counts[j].first] < min_distance && comm_cost[sorted_fpga_ids[thread_id].back()][connection_counts[j].first] != -1) {
                        min_distance = comm_cost[sorted_fpga_ids[thread_id].back()][connection_counts[j].first];
                        min_distance_id = j;
                    }
                }
                if(min_distance_id != -1) {
                    fpga_used[connection_counts[min_distance_id].first] = true;
                    sorted_fpga_ids[thread_id].push_back(connection_counts[min_distance_id].first);
                }
                else{
                    for(int j = 0; j < fpga_num; ++j) {
                        if(fpga_used[connection_counts[j].first]) {
                        ...
                        sorted_fpga_ids[thread_id].push_back(connection_counts[j].first);
    ...
    void sort_fpgas() {
        ...
        for(int thread_id = 0; thread_id < thread_num; ++thread_id) {
            resort_fpgas(connection_counts, thread_id);
        }
    }
    ...
    void backtrack(int node_sorted_index,
        ...
        for (auto f_id : sorted_fpga_ids[thread_id]) {
