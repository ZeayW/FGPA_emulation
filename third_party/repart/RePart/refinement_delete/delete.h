        ...
        while(max_replication_fpga_id != -1 && max_master_id != -1 && count < 1000000){
            ...
            if(max_replication_fpga_id != -1 && max_master_id != -1) {
                ...
                if (canDelete(max_master_id, max_replication_fpga_id, fpgas_comm_change, hyperedges_target_pins_in_part_new)) {
                    ...
                    updateNeighbourNodes(max_master_id, max_replication_fpga_id, gain_threshold);
                    ++count;
                }
            }
        }
...
private:
    ...
    void calculateGain(int gain_threshold) {
        for(const auto& hn: hypergraph.hypernodes) {
            if(hn.deleted) {
                continue;
            }
            int master_id = hn.id;
            int master_fpga_id = hn.label;
            ...
            for(const auto& replication_fpga_id: hn_replication_fpga_labels) {
                if(!checkHopDistance(master_id, replication_fpga_id)) {
                    continue;
                }
    ...
    void updateGainNode(int master_id, int replication_fpga_id) {
        int gain_pos = calculateGain_Positive(master_id, replication_fpga_id);
        int gain_neg = calculateGain_Negative(master_id, replication_fpga_id);
        int gain_new = gain_pos - gain_neg;
        if(gain_priority_queues[replication_fpga_id].contains(master_id)) {
            gain_priority_queues[replication_fpga_id].updateKey(master_id, gain_new);
        } else {
            if(!checkHopDistance(master_id, replication_fpga_id)) {
                return ;
            }
            gain_priority_queues[replication_fpga_id].push(master_id, gain_new);
        }
    }

    void updateNeighbourNodes(int master_id, int replication_fpga_id, int gain_threshold) {
        for(const auto& he_id: hypergraph.hypernodes[master_id].incident_nets_as_source) {
            const Hyperedge& he = hypergraph.hyperedges[he_id];
            for(int i=he._begin+1; i<he._begin+he._size; ++i) {
                ...
                for(auto& j:target_hypernode.replication_fpga_labels) {
                    if(j == replication_fpga_id) {
                        updateGainNode(neighbour_master_id, j);
                    }
                }
            }
        }
