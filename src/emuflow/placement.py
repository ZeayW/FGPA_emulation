...
class Placement:
        ...
        with path.open("r", encoding="utf-8") as stream:
            for line_number, raw_line in enumerate(stream, start=1):
                ...
                if instance not in cell_types:
                    raise ImportError(
                        f"{path}:{line_number}: unknown instance {instance!r}"
                    )
                if instance in seen:
                    raise ImportError(
    ...
    def to_xdc(self) -> str:
        ...
        for index, cell in enumerate(
            ...
            expression = re.escape(cell["instance"]).replace(r"/", "/")
            lines.extend(
                [
                    f"set {variable} [get_cells -hier -regexp "
                    f"{{^{expression}$}}]",
                    f"if {{[llength ${variable}] != 1}} {{",
                    f'  error "EmuFlow expected exactly one cell named '
                    f'{cell["instance"]}"',
                    "}",
