...
public:
    ...
    void Solve(int gain_threshold) {
        ...
        std::cout << "Calculate initial delete gain finished\n";
...
private:
    ...
    std::pair<bool, int> calculateGain_Negative(int master_id, int replication_fpga_id) {
        int gain_neg = 0;
        ...
        bool check_hop_distance = !find_target_pin_in_replication_fpga;
        if(!check_hop_distance) {
            check_hop_distance = fpga_manager.getHopDistance(master_fpga_id, replication_fpga_id) >= 0;
        }
        
        return std::make_pair(check_hop_distance, gain_neg);
    }
    ...
    void calculateGain() {
        for(const auto& hn: hypergraph.hypernodes) {
            ...
            for(const auto& replication_fpga_id: hn_replication_fpga_labels) {
                const auto& [check_hop_distance, gain_neg] = calculateGain_Negative(master_id, replication_fpga_id);
                if(!check_hop_distance) {
                    continue;
                }
    ...
    void deleteNode(int master_id, int replication_fpga_id, int gain, std::unordered_map<int, int>& fpgas_comm_change, std::unordered_map<int, std::vector<int>>& hyperedges_target_pins_in_part_new) {
        const int master_fpga_id = hypergraph.hypernodes[master_id].label;
        std::cout << "Deleting node " << master_id+1 << " from FPGA " << master_fpga_id+1 << " replicated to FPGA " << replication_fpga_id+1 << " with gain " << gain << std::endl;
