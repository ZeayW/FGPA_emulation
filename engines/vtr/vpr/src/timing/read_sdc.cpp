...
class SdcParseCallback : public sdcparse::Callback {
  ...
  private:
    ...
    LaunchCaptureEdgeTimes calculate_launch_to_capture_edge_times(tatum::DomainId launch_domain, tatum::DomainId capture_domain) const {
        // Keep the edge arithmetic in 64 bits.  VPR's SDC time unit is ns and
        // CLOCK_SCALE converts it to ps; long virtual/emulation clock periods
        // can therefore legitimately exceed INT_MAX after scaling even when
        // the unscaled value and the resulting setup/hold constraints are
        // perfectly valid.
